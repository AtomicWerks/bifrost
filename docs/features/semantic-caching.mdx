...
cacheConfig := &semanticcache.Config{
    ...
    CacheByVirtualKey: bifrost.Ptr(false),  // Cache by virtual key instead of provider instance (default: false)
}
...
cacheConfig := &semanticcache.Config{
    ...
    CacheByVirtualKey: bifrost.Ptr(false),
}
...
The `cache_by_virtual_key` option enables caching by virtual key (x-bf-vk header) instead of the backend provider instance. This ensures cache hits when load balancing across multiple provider instances.

```json
{
  "cache_by_virtual_key": false  // Cache by virtual key instead of provider instance (default: false)
}
```

**Why use virtual key caching:**
- **Load Balancing**: When using multiple provider instances behind a load balancer, each instance has different cache storage
- **Virtual Keys**: The `x-bf-vk` header identifies a logical "virtual" provider that spans across instances
- **Cross-Instance Hits**: Enable this to share cache entries across all instances using the same virtual key

**When to enable (`true`):**
- Running Bifrost with multiple provider instances behind a load balancer
- Using virtual keys (x-bf-vk header) to identify logical provider instances
- Want cache hits regardless of which instance handles the request

**When to disable (`false`, default):**
- Single instance deployments
- Using provider instance-specific caching
- Existing configurations that should not change behavior
